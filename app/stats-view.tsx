...
type StatsSortKey = "name" | "games" | "winRate" | "averageKda" | "kills" | "deaths" | "assists" | "kda" | "averageGold" | "mainLane" | "secondaryLane";
...
export function StatsView({ data, teamMap, onOpenPlayer }: { data: StatsData; teamMap: Map<string, StatsTeam>; onOpenPlayer: (userId: string | null, accountName: string) => void }) {
  ...
  const playerRows = useMemo(() => [...basePlayerRows].sort((a, b) => {
    ...
    else if (sort.key === "kills") compared = a.kills - b.kills;
    else if (sort.key === "deaths") compared = a.deaths - b.deaths;
    else if (sort.key === "assists") compared = a.assists - b.assists;
  ...
  return <section className="page-section stats-page">
    ...
    <article className="panel stats-table-panel"><div className="section-heading"><div><p className="eyebrow">PLAYER LEADERBOARD</p><h2>계정별 기록</h2></div><span className="stats-sort-help">헤더를 누르면 정렬이 바뀝니다</span></div><div className="stats-table" role="table" aria-label="계정별 경기 통계"><div className="stats-table-head" role="row">{sortHeader("name", "계정")}{sortHeader("games", "경기(승-패)")}{sortHeader("winRate", "승률")}{sortHeader("averageKda", "평균 K/D/A")}{sortHeader("kills", "총 킬")}{sortHeader("deaths", "총 데스")}{sortHeader("assists", "총 어시")}{sortHeader("kda", "KDA")}{sortHeader("averageGold", "평균 골드")}{sortHeader("mainLane", "주라인")}{sortHeader("secondaryLane", "부라인")}</div>{playerRows.map((row) => {
      ...
      return <div className={`stats-table-row${primaryTeam ? " team-colored" : ""}${visibleTeams.length > 1 ? " multi-team" : ""}`} style={primaryTeam ? { "--stats-team-color": primaryTeam.color } as CSSProperties : undefined} role="row" key={row.key}><button type="button" className={accountClassName} onClick={() => onOpenPlayer(row.userId, row.name)}><span className="stats-account-identity"><strong>{row.name}</strong>{!row.userId && <small>선수 미연동</small>}</span>{visibleTeams.length > 0 && <span className="stats-account-teams">{visibleTeams.map((team) => <span className="stats-team-badge" style={{ "--stats-team-color": team.color } as CSSProperties} key={team.id}><i aria-hidden="true" />{team.name}</span>)}</span>}</button><span>{row.games} ({row.wins}-{row.losses})</span><span>{Math.round(row.winRate)}%</span><span>{row.averageKills.toFixed(1)} / {row.averageDeaths.toFixed(1)} / {row.averageAssists.toFixed(1)}</span><span>{row.kills.toLocaleString()}</span><span>{row.deaths.toLocaleString()}</span><span>{row.assists.toLocaleString()}</span><b>{row.kda.toFixed(2)}</b><span>{Math.round(row.averageGold).toLocaleString()}</span><span>{row.mainLane ? `${positionLabel(row.mainLane.lane, true)} · ${row.mainLane.games}경기` : "-"}</span><span>{row.secondaryLane ? `${positionLabel(row.secondaryLane.lane, true)} · ${row.secondaryLane.games}경기` : "-"}</span></div>;
    })}</div></article>
    {data.tournament?.competitionKind !== "scrim_season" && <div className="team-stat-grid">{teamRows.map((row) => { const playerSets = row.games * 5; return <article className="panel" key={row.team.id} style={{ "--team-color": row.team.color } as CSSProperties}><header><TeamMark team={teamMap.get(row.team.id)} /><div><span>{row.games}경기 · {row.wins}승</span><h3>{row.team.name}</h3></div><strong>{Math.round(row.wins / row.games * 100)}%</strong></header><div><span>선수 1명·세트당 평균 K/D/A</span><b>{(row.kills / playerSets).toFixed(1)} / {(row.deaths / playerSets).toFixed(1)} / {(row.assists / playerSets).toFixed(1)}</b></div><div><span>팀 누적 K/D/A</span><b>{row.kills.toLocaleString()} / {row.deaths.toLocaleString()} / {row.assists.toLocaleString()}</b></div><div><span>팀 종합 KDA</span><b>{((row.kills + row.assists) / Math.max(1, row.deaths)).toFixed(2)}</b></div><div><span>팀 평균 골드</span><b>{Math.round(row.gold / row.games).toLocaleString()}</b></div></article>; })}</div>}
  </section>;
}
