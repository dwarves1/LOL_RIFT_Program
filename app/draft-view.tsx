...
export function DraftView({ data, teamMap, busy, command }: {
  data: { viewer: Viewer; draftSessions: DraftSession[]; practiceDrafts: DraftSession[]; matches: Match[] };
  ...
  useEffect(() => { fetch("/api/champions").then((response) => response.json()).then((payload: { champions: Champion[] }) => setChampions(payload.champions)).catch(() => setChampions([])); }, []);
